...
| **Instance Configuration** | Deployment-level configuration | Must declare storage roots, rolling archives, executor defaults, facility metadata, and optional custom generator modules. |
...
- rolling archive references or explicit input folder references
- input definitions
- output definitions
- output publication policy, if outputs should be copied back to a rolling archive
...
2. Product candidates must then be evaluated according to the ordered `product_folders` list.
3. Auxiliary candidates must then be evaluated according to the ordered `aux_folders` list.
4. Folder entries that reference `rolling:<archive_id>` must be resolved through instance configuration before scanning.
5. Rolling archive references must preserve their declared order relative to literal filesystem paths.
6. If multiple candidates exist within the same effective precedence level, the station or system policy must define a deterministic tie-break rule.
7. If no explicit tie-break rule is configured, the implementation must use a documented default that is stable across retries.
...
The input selection algorithm must follow the same decision path for products and auxiliaries. Product inputs use the station's `product_folders`; auxiliary inputs use the station's `aux_folders`.

```mermaid
flowchart TD
  A[Declared station input] --> B[Resolve folder references]
  B --> C[Expand rolling archive IDs to paths]
  C --> D[Scan folders in declared order]
  D --> E{Candidate matches file pattern and time margins?}
  E -- No --> F{More folders?}
  F -- Yes --> D
  F -- No --> G{Input mandatory?}
  G -- Yes --> H[Fail preparation]
  G -- No --> I[Record optional input missing]
  E -- Yes --> J{Multiple candidates at same precedence?}
  J -- No --> K[Select candidate]
  J -- Yes --> L[Apply deterministic tie-break rule]
  L --> K
  K --> M[Record manifest entry]
  M --> N[Symlink selected file into working_root/input]
```
...
joborder_id: joborder-run-sce2-000043
task_id: task-20250703-111539-sce2
...
The system-default working-root path generator must use the configured `working_root_base`. A custom working-root generator may be configured at the instance level. It must return a path under an approved shared-storage root, must be deterministic for the provided run context, and must not collide with an existing run. If a custom generator returns an invalid path, preparation must fail before any scheduler job is submitted.
...
- The backend must record publication metadata including source artifact, target rolling archive, target path, publication mode, checksum if available, and timestamp.
