...
2. Each declared input must specify a `category` and a `file_type`.
3. The effective folder search order for an input must be taken from the matching top-level `product_categories` entry.
4. A station may override the top-level folder order for a category through `rolling_folders`.
5. Folder entries that reference `rolling:<archive_id>` must be resolved through instance configuration before scanning.
6. Resolved folder entries must preserve their declared order relative to literal filesystem paths.
7. If multiple candidates exist within the same effective precedence level, the station or system policy must define a deterministic tie-break rule.
8. If no explicit tie-break rule is configured, the implementation must use a documented default that is stable across retries.
...
flowchart TD
  ...
  F --> G
  G --> H[Scan folders in declared order]
  H --> I{Candidate matches file pattern and time margins?}
  I -- No --> J{More folders?}
  J -- Yes --> H
  J -- No --> K{Input mandatory?}
  K -- Yes --> L[Fail preparation]
  K -- No --> M[Record optional input missing]
  I -- Yes --> N{Multiple candidates at same precedence?}
  N -- No --> O[Select candidate]
  N -- Yes --> P[Apply deterministic tie-break rule]
  P --> O
  O --> Q[Record manifest entry]
  Q --> R[Symlink selected file into working_root/input]
```
