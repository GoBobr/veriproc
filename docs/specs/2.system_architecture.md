...
naming:
  ...
  working_root:
    ...
    collision_suffix: "-{attempt_suffix}"
  filenames:
...
```text
/shared/veriproc/work/station-a/station-a-20250703T101218019-a1b2c3/run-20260506T071500001234Z
```

The working-root path is not the run identity. The canonical run key is `(task_id, retry_index)`. Human-readable path segments are operational presentation values derived from validated metadata and must not become foreign keys, provenance targets, API identities, or deduplication owners.

The system-default or configured working-root generator may use station ID, task start time, task end time, retry index, run creation or preparation timestamp, and another deterministic per-run uniqueness component. The generator must not require the preferred display form `task_id/rN` to be visible in the path. If two logical runs would otherwise produce the same readable path, the generator must add a deterministic disambiguation suffix. This suffix is collision avoidance only and must not be treated as a second identifier.
