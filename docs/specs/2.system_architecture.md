...
The execution adapter must inject a predefined runtime environment into the job process. The baseline environment should include at least `VERIPROC_WORKING_ROOT`, `VERIPROC_STATION_ID`, `VERIPROC_TASK_ID`, `VERIPROC_RETRY_INDEX`, `VERIPROC_RUN_REF`, and `VERIPROC_JOBORDER_PATH`. `VERIPROC_RUN_REF` should use the preferred display convention, for example `task-20250703T111239000-a7sf7f/r5`, and is a convenience representation of the canonical composite `(task_id, retry_index)`. Additional deployment-specific variables may be provided, but they must not replace the job order as the primary execution contract.
