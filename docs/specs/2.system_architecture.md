...
- By default, the publication target path must place the published file at the root of the selected archive using its effective published filename. VeriProc must not implicitly prepend `station_id` or any other derived directory segment.
