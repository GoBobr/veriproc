...
- a lightweight client-side router, for example `TanStack Router` or a small hash/history router
- a lightweight polling and refresh layer, for example `TanStack Query` or explicit polling hooks
...
- `[R]` running run, rendered in an active accent color such as blue
- `[W]` queued, accepted, or dispatching run, rendered in a neutral pending color such as slate or gray
- `[F]` failed runs, and failed tasks for which no run was ever created, must remain visible in red until an operator explicitly hides them from the dashboard
...
4. failed or cancelled runs, and immediately-failed tasks with no run, that are still visible because they have not been hidden by an operator, newest first by terminal timestamp when available
5. empty slots

If a slot occupant has no terminal timestamp or task-creation timestamp available from the upstream contract, the console gateway may use another stable deterministic fallback such as upstream list order or retry index.
...
### 8.5.4 Station Activity View

Clicking a station label on the dashboard must open a station activity page in a new browser tab. The station activity page must display a paginated, sortable table of all runs for that station.

The table must contain at minimum the following columns:

- `task_id` — the task identifier, linked to the task detail page
- `retry_index` — the run's retry index, rendered as a coloured badge
- `created_at` — the run creation timestamp
- `working_root` — the run's working root directory path
- `state` — a coloured status button that opens the task detail page in a new browser tab when clicked

Adjacent runs belonging to the same task should be visually grouped.

The table must support server-side sorting by `run_id` (default), `created_at`, and `task_id`, with ascending and descending direction. Clicking a column header must toggle the sort direction.

The table must support cursor-based pagination with a configurable page size. The default page size should be 50 and should be configurable via the `ui.station_runs_page_size` configuration key. A page-size selector must allow the operator to choose between 10, 25, 50, and 100 runs per page.

Status colours must be consistent with the dashboard slot colour scheme defined in §8.4.2.

The console gateway must expose a station runs endpoint that proxies run-list queries to the upstream `GET /api/v1/runs` endpoint, scoped to the station identified by the path parameter. The endpoint must pass through pagination, sorting, and state filter query parameters and return the upstream response including the `next_cursor` pagination token.
...
- console-local authentication sources such as inline tokens, environment variables, or file-backed token lists
...
- dashboard defaults such as visible slot count, completed-run visibility timeout, refresh interval, and station activity page size
- path and migration policy for the console SQLite database
- optional frontend bundle directory, session settings, and audit-retention settings
...
ui:
  ...
  station_runs_page_size: 50

db:
  dsn: file:veriproc-console.db

auth:
  tokens_file: /etc/veriproc/console-tokens.yaml

webapp_dir: /opt/veriproc/console-webapp/dist
...
- `users` or an equivalent subject registry
- `api_tokens` or an equivalent credential table that stores the effective console role together with the authenticated subject; session tables are optional
...
Implementations should also emit a structured process log entry for each mutating action so operator activity is visible in the console service log at normal operational log levels.
...
- `POST /api/console/instances/{instance_id}/tasks/{task_id}/runs/{retry_index}/hide?station_id=...`
...
- `GET /api/console/instances/{instance_id}/stations/{station_id}/runs?limit=...&cursor=...&sort=...&order=...`
  ...
  "stations": [
    {
      ...
      "slots": [
        ...
        {"kind": "failed", "task_id": "task-9", "retry_index": 1},
        {"kind": "failed", "task_id": "task-10", "retry_index": 0},
...
- list failed tasks by station and terminal state so the console can surface immediately-failed tasks that never created a run
- pause a station
- unpause a station
...
- `POST /api/v1/tasks`
- `GET /api/v1/tasks` with filters such as `station_id`, `state`, and `limit`
...
If the upstream station summary is run-derived and therefore omits failed tasks that never created a run, the console gateway may supplement the failed-slot population from `GET /api/v1/tasks` or an equivalent authoritative task-list endpoint before returning the dashboard response.
...
When immediately-failed tasks are not represented in the upstream station summary because no run exists, the console gateway may enrich the failed slot set from an authoritative upstream task listing, keyed by `(task_id, retry_index)` or another stable console identity.
